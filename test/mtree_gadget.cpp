...
bool test_pmtree()
{
    ...
    // There is no need to extract as field_variable does that for us
...
bool test_pmtree_path()
{
    ...
    pb.set_input_sizes(DIGEST_VARS);
    MTree_Gadget<FieldT, GadHash> gadget{pb, out, trans, other, 0, FMT("merkle_tree")};
...
static bool run_tests()
{
    ...
    std::cout.flush();
    {
        check = test_mtree_path<TREE_HEIGHT, Sha256, GadSha256>();
    }
    std::cout << check << '\n';
    all_check &= check;*/
...
/*
    ...
    std::cout.flush();
    {
        check = test_pmtree_path<TREE_HEIGHT, Mimc256, GadMimc256>();
    }
    std::cout << check << '\n';
    all_check &= check;*/
